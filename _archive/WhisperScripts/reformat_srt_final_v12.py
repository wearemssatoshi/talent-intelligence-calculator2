...
def create_final_subtitles(input_path, output_path):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        for i, sub in enumerate(subtitles, 1):
            ...
            # PDF and User Preference based corrections
            text = re.sub(r'[、。]', '', text)
            text = text.replace('リエ', 'りえ')
            text = text.replace('子供', '子ども') # Re-added based on user feedback
            text = text.replace('嫉妬', '叱咤')
            text = text.replace('めどくさい', 'めんどくさい')
            text = text.replace('マスマス', 'ますます')
            text = text.replace('鉄屋', '徹夜')
            text = text.replace('力無', '力む')
            text = text.replace('企業をしたり', '起業したり')
