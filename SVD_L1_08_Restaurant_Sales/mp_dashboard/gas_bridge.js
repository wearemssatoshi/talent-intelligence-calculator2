...
const GAS_BRIDGE = (() => {
    ...
    // ── POST: Import (Sheet-level bulk import) ──
    async function importData(sheetName, rows, user) {
        if (!GAS_URL) return { status: 'error', message: 'GAS URL not set' };
    ...
    // ── AI Proxy: GAS経由でGemini APIを呼ぶ ──
    async function askAI(question, context) {
        if (!GAS_URL) return null;
        try {
            const resp = await fetch(`${GAS_URL}?action=askAI&q=${encodeURIComponent(question)}&context=${encodeURIComponent(context || '')}${tokenParam()}`, {
                redirect: 'follow'
            });
            const data = await resp.json();
            ...
            console.warn('[GAS] askAI failed:', e.message);
            return null;
        }
    }
    ...
    return {
        ...
        loadAll, loadDate, loadRange, loadOnHand, askAI,
        save, bulkSave, importData, processQueue, addToken, tokenParam,
